...
import {
  ...
  updateDoc,
  getDoc,
  setDoc, // 🚀 1. เพิ่มคำนี้เข้าไปตรงนี้ครับ
  serverTimestamp,
  query,
  orderBy,
  where,
...
// ==========================================
// 🚀 Helper: จัดฟอร์แมตเวลาสำหรับ Tracking (DD/MMM/YYYY H:m)
// ==========================================
...
const AppIcon: React.FC<AppIconProps> = ({
  ...
  >
    ...
    <span className="text-white font-bold text-sm md:text-lg tracking-wide drop-shadow-sm px-2 relative z-10 text-center">
      {label}
    </span>
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 flex flex-col">
      <div className="bg-white border-b border-slate-200 px-4 py-3 flex items-center justify-between sticky top-0 z-30 shadow-sm">
        ...
        <button
          ...
          className="p-2 hover:bg-red-50 text-red-500 rounded-xl flex items-center gap-2 font-bold transition-colors text-sm"
        >
          <LogOut size={18} />
...
// ==========================================
// 🌟 หน้า Acknowledge (กดรับทราบแล้วสร้าง Tracking อัตโนมัติ)
// ==========================================
// ==========================================
// 🌟 หน้า Acknowledge (หน้ารับทราบ Magic Link)
// ==========================================
const AcknowledgeView = () => {
  ...
  // 🚀 เพิ่มตัวล็อก ป้องกัน React StrictMode รันคำสั่งเบิ้ล 2 รอบ
  ...
  useEffect(() => {
    ...
    const confirmAcknowledge = async () => {
      // 🚀 ถ้าเคยรันไปแล้วให้ข้ามเลย ป้องกันการสร้าง Tracking ซ้ำ
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          if (!data.acknowledged) {
            // 1. อัปเดตสถานะใน Log Database ว่า "รับทราบแล้ว"
            await updateDoc(docRef, {
              acknowledged: true,
              acknowledged_at: serverTimestamp(),
            });

            // 2. อัปเดต Product แต่ละตัว
              ...
              const updatePromises = data.items.map((item: any) => {
                if (item.product_id) {
                  return updateDoc(doc(db, 'products', item.product_id), {
                    ...
                    acknowledged_at: serverTimestamp(),
                  }).catch((e) => console.log('Failed to update product:', e));
                }
            ...
            // 3. สร้าง Tracking อัตโนมัติ (เริ่ม Step 1: เตรียมจัดส่ง)
      ...
      <div className="bg-white p-8 md:p-12 rounded-3xl shadow-2xl max-w-md w-full text-center border border-slate-100">
        ...
        {status === 'success' && (
          <div className="flex flex-col items-center animate-slide-down">
            <CheckCircle
              className="text-emerald-500 mb-6 drop-shadow-md"
              size={80}
            />
            <h2 className="text-2xl font-black text-slate-800 mb-2">
              รับทราบการจัดส่งเรียบร้อย!
            </h2>
            <p className="text-slate-600 mb-6 leading-relaxed">
              ขอบคุณ <b>{supplierName}</b>
              <br />
              ระบบได้บันทึกการยืนยันและสร้างเลข <b>Tracking</b>{' '}
              สำหรับติดตามการจัดส่งของท่านแล้วครับ
            </p>
            <button
              onClick={() => window.close()}
              className="px-8 py-3 bg-slate-800 hover:bg-slate-900 text-white font-bold rounded-xl shadow-lg transition-colors w-full"
            >
              ปิดหน้าต่างนี้
            </button>
          </div>
        ...
        {status === 'error' && (
          <div className="flex flex-col items-center animate-slide-down">
            <AlertTriangle
              className="text-red-500 mb-6 drop-shadow-md"
              size={80}
            />
            <h2 className="text-2xl font-black text-slate-800 mb-2">
              ไม่พบข้อมูล
            ...
            <p className="text-slate-600">
              ลิงก์อาจหมดอายุ หรือมีการยืนยันไปแล้วก่อนหน้านี้
            </p>
          </div>
        )}
      </div>
...
const LoginView = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // 🚀 รหัสลับแอดมิน (เผื่อระบบยังไม่มี User เลย ให้ใช้อันนี้เข้าได้เสมอ)
      if (email === 'admin@vmi.com' && password === 'admin1234') {
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          <div>
            ...
            <input
              ...
              className="w-full border border-slate-300 px-4 py-3 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 transition-shadow bg-slate-50 focus:bg-white"
              placeholder="admin@vmi.com"
            />
          </div>
          <div>
            <label className="block text-xs font-bold text-slate-700 mb-1">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3.5 rounded-xl shadow-lg shadow-blue-500/30 transition-all flex justify-center items-center gap-2"
          >
            {loading ? (
...
const HomeView = () => {
  ...
  useEffect(() => {
      ...
      (snapshot) => {
        setLowStockCount(
          snapshot.docs.filter((doc) => {
            const d = doc.data();
            return (
              d.available_qty <= d.min_stock && d.min_stock > 0 && !d.is_emailed
            );
          }).length
        );
      }
    ...
    // 🚀 แจ้งเตือน Tracking ที่กำลังวิ่งอยู่ (สถานะ 1, 2, 3)
    const unsubTrack = onSnapshot(
      query(collection(db, 'tracking')),
      (snapshot) => {
        setActiveTrackingCount(
          snapshot.docs.filter((doc) => {
            const d = doc.data();
            return d.current_status >= 1 && d.current_status < 4;
          }).length
        );
      }
    );
...
// ==========================================
// 🌟 หน้า Tracking View (อัปเดตสถานะ Line OA) 🚀
// ==========================================
const TrackingView = () => {
  ...
  const StatusStepper = ({ currentStatus, timestamps }: any) => {
    const steps = [
      { id: 1, label: 'เตรียมจัดส่ง' },
      { id: 2, label: 'กำลังจัดส่ง' },
      { id: 3, label: 'ถึงโรงงาน' },
      { id: 4, label: 'โรงงานรับสินค้า' },
    ];
              ...
              <div className="flex flex-col items-center flex-1 relative z-10">
                ...
                <div className="mt-2 text-center w-full flex flex-col items-center">
                  ...
                  <p className="text-[10px] text-slate-500 font-mono mt-0.5 leading-tight">
                    {formatTrackingDate(time)}
                  </p>
                </div>
              </div>
              {index < steps.length - 1 && (
                <div
                  className={`flex-1 h-1 rounded-full -mx-6 z-0 ${
                    currentStatus > step.id ? 'bg-cyan-500' : 'bg-slate-200'
                  }`}
                />
              )}
            </React.Fragment>
        ...
        <div className="space-y-6 flex flex-col h-full relative">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">
              ติดตามสถานะการจัดส่ง (จำลองการรับค่าจาก Line OA Flex Message)
            </p>
          </div>
              ...
              trackings.map((track) => (
                ...
                >
                  ...
                  <StatusStepper
                    ...
                    timestamps={track.status_timestamps}
                  />

                  <div className="mt-6 bg-slate-50 p-3 rounded-xl border border-slate-100">
                    <p className="text-xs font-bold text-slate-500 mb-2">
                      รายการสินค้าในเที่ยวนี้ ({track.items?.length || 0}):
                    </p>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                      {track.items?.map((item: any, idx: number) => (
                        <div
                          key={idx}
                          className="bg-white px-3 py-2 rounded-lg border border-slate-200 text-sm flex justify-between items-center shadow-sm"
                        >
                          <span
                            className="font-mono font-bold text-slate-700 truncate mr-2"
                            title={item.description}
                          >
                            {item.part_no}
                          </span>
                          <span className="text-red-600 font-bold bg-red-50 px-2 rounded">
                            ขาด{' '}
                            {(
                              (item.min || 0) - (item.qty || 0)
                            ).toLocaleString()}
                          </span>
                        </div>
                      ))}
                  ...
                  <div className="mt-4 pt-4 border-t border-slate-100 flex flex-wrap gap-2 items-center">
                    ...
                    <button
                      onClick={() => simulateLineOAUpdate(track.id, 2)}
                      disabled={track.current_status >= 2}
                      className="text-[11px] px-3 py-1.5 rounded-lg bg-blue-50 text-blue-700 font-bold border border-blue-200 disabled:opacity-50 transition-colors"
                    >
                      กด กำลังจัดส่ง
                    </button>
                    <button
                      onClick={() => simulateLineOAUpdate(track.id, 3)}
                      disabled={track.current_status >= 3}
                      className="text-[11px] px-3 py-1.5 rounded-lg bg-orange-50 text-orange-700 font-bold border border-orange-200 disabled:opacity-50 transition-colors"
                    >
                      กด ถึงโรงงาน
                    </button>
                    <button
                      onClick={() => simulateLineOAUpdate(track.id, 4)}
                      disabled={track.current_status >= 4}
                      className="text-[11px] px-3 py-1.5 rounded-lg bg-emerald-50 text-emerald-700 font-bold border border-emerald-200 disabled:opacity-50 transition-colors"
                    >
                      กด โรงงานรับสินค้า
                    </button>
                  </div>
...
const DashboardView = () => {
  ...
  const [viewingProductList, setViewingProductList] = useState<{
    title: string;
    items: any[];
  } | null>(null);
  const [viewingConsumptionPart, setViewingConsumptionPart] = useState<
    any | null
  >(null);
  const [viewYear, setViewYear] = useState<string>(currentYear.toString());

  useEffect(() => {
    const unsubProd = onSnapshot(query(collection(db, 'products')), (s) =>
      setProducts(s.docs.map((d) => ({ id: d.id, ...d.data() })))
    );
    const unsubSup = onSnapshot(query(collection(db, 'suppliers')), (s) =>
      setSuppliers(s.docs.map((d) => ({ id: d.id, ...d.data() })))
    );
    const unsubLog = onSnapshot(
      query(collection(db, 'email_logs'), orderBy('sent_at', 'desc')),
  ...
  const zeroStockProducts = products.filter(
    (p) => p.available_qty <= 0 && p.min_stock > 0
  );
  const lowStockProducts = products.filter(
    (p) =>
      p.available_qty <= p.min_stock && p.available_qty > 0 && p.min_stock > 0
  );
  const normalStockProducts = products.filter(
    (p) => p.available_qty > p.min_stock || p.min_stock === 0
  );

  const uniqueSuppliers = new Set(
    suppliers.map((s) => s.vendor_no).filter(Boolean)
  );
  ...
  const stockHealthData = [
    {
      name: 'Normal (ปกติ)',
      value: normalStockProducts.length,
      color: '#10b981',
      items: normalStockProducts,
    },
    {
      name: 'Low Stock (ต่ำกว่า Min)',
      value: lowStockProducts.length,
      color: '#f59e0b',
      items: lowStockProducts,
    },
    {
      name: 'Out of Stock (ของหมด)',
      value: zeroStockProducts.length,
      color: '#ef4444',
      items: zeroStockProducts,
    },
  ];
  ...
  const getForecastOrderDate = (
    availableQty: number,
    minStock: number,
    avgWeek: number
  ) => {
    if (availableQty <= minStock)
      return { text: 'สั่งด่วน (ของขาด)', urgent: true };
    ...
    return {
      text: orderDate.toLocaleDateString('en-GB', {
        day: '2-digit',
        month: 'short',
        year: 'numeric',
      }),
  ...
  const urgentItems = products
    ...
    .map((p) => {
      ...
      const orderDays = getForecastOrderDays(
        p.available_qty,
        p.min_stock,
        avgWeek
      );
  ...
  const KpiCard = ({
    ...
    <div
      ...
      className={`bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-4 transition-all ${
        onClick
          ? 'cursor-pointer hover:shadow-md hover:scale-105 hover:border-blue-300'
          : 'hover:shadow-md'
      }`}
    >
      ...
      <div className="h-full flex flex-col gap-6 overflow-y-auto custom-scrollbar pr-2 pb-6">
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 shrink-0">
          ...
          <KpiCard
            onClick={() =>
              setViewingProductList({
                title: 'Low Stock (ทั้งหมดที่ต่ำกว่า Min)',
                items: [...lowStockProducts, ...zeroStockProducts],
              })
            }
            title="Low Stock"
            value={(
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 shrink-0">
          <div className="bg-white border border-slate-200 p-5 rounded-2xl shadow-sm flex flex-col">
            <h4 className="font-bold text-lg text-slate-700 mb-4 flex items-center gap-2">
              <Activity className="text-emerald-500" /> Stock Health Status
            </h4>
          ...
          <div className="bg-white border border-slate-200 p-5 rounded-2xl shadow-sm flex flex-col">
            <h4 className="font-bold text-lg text-slate-700 mb-4 flex items-center gap-2">
              <BarChart3 className="text-blue-500" /> Top 5 Suppliers
              (ของขาดมากสุด)
            </h4>
              ...
              {supplierChartData.length === 0 ? (
                ...
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <Tooltip
                      cursor={{ fill: '#f1f5f9' }}
                      formatter={(value) => [`${value} รายการ`, 'ของขาด']}
                    />
                    <Bar
                      dataKey="count"
                      fill="#f59e0b"
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 shrink-0">
          <div className="bg-white border border-slate-200 p-5 rounded-2xl shadow-sm">
            <h4 className="font-bold text-lg text-slate-700 mb-4 flex items-center gap-2">
              <AlertCircle className="text-red-500" /> Watchlist: สั่งของด่วน
            </h4>
                        ...
                        <td className="p-3 text-center">
                          {p.orderDays <= 0 ? (
                            ...
                            <span className="bg-orange-100 text-orange-700 px-2 py-1 rounded text-[10px] font-bold">
                              สั่งใน {Math.ceil(p.orderDays)} วัน
                            </span>
                          )}
                        </td>
                      </tr>
          ...
          <div className="bg-white border border-slate-200 p-5 rounded-2xl shadow-sm">
            <h4 className="font-bold text-lg text-slate-700 mb-4 flex items-center gap-2">
              <Clock className="text-indigo-500" /> Recent Replenishments
            </h4>
            <div className="overflow-hidden rounded-xl border border-slate-100">
              <table className="w-full text-left text-sm">
                <thead className="bg-slate-50 text-slate-600 text-xs uppercase font-bold">
                  <tr>
                    <th className="p-3">เวลาที่ส่ง</th>
                    <th className="p-3">Supplier</th>
                    ...
                    logs.slice(0, 5).map((log) => (
                      <tr key={log.id} className="hover:bg-slate-50">
                        <td className="p-3 font-mono text-slate-500 text-xs">
                          {log.sent_at?.toDate
                            ? log.sent_at.toDate().toLocaleString('th-TH', {
                                dateStyle: 'short',
                                timeStyle: 'short',
                              })
                            : '...'}
                        </td>
                        <td className="p-3 font-bold text-indigo-700 truncate max-w-[150px]">
                          {log.supplier_name}
                        </td>
                        <td className="p-3 text-center">
                          <span className="bg-slate-100 text-slate-600 px-2 py-0.5 rounded text-xs font-bold">
                            {log.items?.length || 0} รายการ
                          </span>
                        </td>
                      </tr>
                    ))
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col max-h-[80vh] animate-slide-down">
              <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                <h3 className="font-bold text-lg text-slate-800 flex items-center gap-2">
                  ...
                  รายการสินค้า: {viewingProductList.title}
                </h3>
                <button
                  onClick={() => setViewingProductList(null)}
              ...
              <div className="p-0 overflow-y-auto custom-scrollbar flex-1">
                <table className="w-full text-left whitespace-nowrap text-sm">
                  <thead className="bg-slate-100 text-slate-600 text-[11px] uppercase font-bold sticky top-0 shadow-sm">
                    <tr>
                      <th className="p-3 pl-6">Part No</th>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {viewingProductList.items.length === 0 ? (
                      <tr>
                        <td
                          colSpan={5}
                      ...
                      viewingProductList.items.map((p) => (
                        <tr
                          key={p.id}
                          onClick={() => handleRowClick(p)}
                          className="hover:bg-slate-50 cursor-pointer transition-colors"
                        >
                          <td className="p-3 pl-6 font-mono font-bold text-slate-700 flex items-center gap-2">
                            {p.part_no}{' '}
                            {p.is_acknowledged ? (
                              <span className="text-[9px] bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded border border-emerald-200 font-bold">
                                Acked
                              </span>
                            ) : p.is_emailed ? (
                              <span className="text-[9px] bg-orange-100 text-orange-700 px-1.5 py-0.5 rounded border border-orange-200 font-bold">
                                Sent
                              </span>
                            ) : null}
                          </td>
                          <td className="p-3 text-slate-800">
                            <div className="truncate w-48">{p.description}</div>
                          </td>
                          ...
                          <td className="p-3 text-center">
                            {p.available_qty <= p.min_stock ? (
                              <span className="px-2 py-1 bg-red-100 text-red-700 text-[10px] rounded font-bold">
                                ต่ำกว่ากำหนด
                              </span>
                            ) : (
                              <span className="px-2 py-1 bg-green-100 text-green-700 text-[10px] rounded font-bold">
                                ปกติ
                              </span>
                            )}
                          </td>
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl overflow-hidden flex flex-col max-h-[90vh] animate-slide-down">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="font-bold text-xl text-slate-800">
                      {viewingConsumptionPart.product.part_no}
                    </h3>
                    <p className="text-sm text-slate-500">
                      {viewingConsumptionPart.product.description}
                    </p>
                  </div>
                </div>
                <button
                  onClick={() => setViewingConsumptionPart(null)}
                  className="p-2 hover:bg-slate-200 rounded-full text-slate-500 transition-colors"
                >
                  <X size={24} />
                </button>
              ...
              <div className="p-6 overflow-y-auto custom-scrollbar flex-1 bg-slate-50/50">
                {(() => {
                  ...
                  const yearData = sup
                    ? sup.yearly_data?.[viewYear] ||
                      sup.monthly_data?.[viewYear] ||
                      {}
                    : {};
                  const values = MONTHS.map((m) => yearData[m] || 0);
                  const total = values.reduce((sum, val) => sum + val, 0);
                  ...
                  const forecast = getForecastOrderDate(
                    ...
                    <div className="space-y-6">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="bg-orange-50 border border-orange-100 p-4 rounded-xl flex justify-between items-center">
                          <div>
                            <p className="text-xs font-bold text-orange-600 mb-1 uppercase tracking-wide">
                              Available Qty (ยอดใช้ได้จริง)
                            </p>
                            <p className="text-3xl font-mono font-extrabold text-orange-800">
                        ...
                        <div
                          ...
                          } border p-4 rounded-xl flex justify-between items-center`}
                        >
                          <div>
                            <p
                      ...
                      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mt-6 gap-4">
                        <div>
                          <h4 className="font-bold text-lg text-slate-700">
                            ประวัติการใช้งาน (Consumption)
                          </h4>
                          {sup && (
                            <p className="text-sm text-emerald-600 font-bold">
                              Supplier: {sup.vendor_name}
                            </p>
                          )}
                        </div>
                        <div className="flex items-center gap-2 bg-emerald-50 border border-emerald-200 px-3 py-1.5 rounded-xl shadow-sm">
                          ...
                          <select
                            className="bg-transparent font-bold text-emerald-800 outline-none text-sm cursor-pointer"
                            value={viewYear}
                      ...
                      <div className="grid grid-cols-3 md:grid-cols-4 xl:grid-cols-6 gap-3">
                        {MONTHS.map((m) => (
                          <div
                            key={m}
                            className="bg-white border border-slate-200 p-3 rounded-xl shadow-sm flex flex-col items-center justify-center"
                          >
                            ...
                            <span className="text-lg font-mono font-bold text-slate-700">
                              {yearData[m] > 0
                                ? yearData[m].toLocaleString(undefined, {
                                    maximumFractionDigits: 2,
                                  })
                                : '-'}
                            </span>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div className="bg-blue-50 border border-blue-100 p-4 rounded-xl">
                          ...
                          <p className="text-xl font-mono font-bold text-blue-800">
                            {avgMonth > 0
                              ? avgMonth.toLocaleString(undefined, {
                        ...
                        <div className="bg-blue-50 border border-blue-100 p-4 rounded-xl">
                          ...
                          <p className="text-xl font-mono font-bold text-blue-800">
                            {avgWeek > 0
                              ? avgWeek.toLocaleString(undefined, {
                                  maximumFractionDigits: 1,
                                })
                              : '0'}
                          </p>
                        ...
                        <div className="bg-red-50 border border-red-100 p-4 rounded-xl">
                          ...
                          <p className="text-xl font-mono font-bold text-red-800">
                            {autoMin > 0
                              ? autoMin.toLocaleString(undefined, {
                                  maximumFractionDigits: 1,
                                })
                              : '0'}
                          </p>
                        ...
                        <div className="bg-emerald-50 border border-emerald-100 p-4 rounded-xl">
                          ...
                          <p className="text-xl font-mono font-bold text-emerald-800">
                            {autoMax > 0
                              ? autoMax.toLocaleString(undefined, {
                                  maximumFractionDigits: 1,
                                })
                              : '0'}
                          </p>
                      ...
                      {!sup && (
                        <div className="text-center p-4 bg-orange-50 text-orange-600 rounded-xl font-bold text-sm">
                          ไม่พบประวัติการใช้งาน (ยังไม่มีการผูก Supplier
                          สำหรับสินค้านี้)
                        </div>
                      )}
                    </div>
              ...
              <div className="px-6 py-4 border-t border-slate-100 bg-white flex justify-end">
                <button
                  onClick={() => setViewingConsumptionPart(null)}
                  className="px-8 py-2.5 bg-slate-800 hover:bg-slate-900 text-white rounded-xl font-bold transition-colors shadow-md"
                >
                  ปิดหน้าต่าง
                </button>
              </div>
...
const WorkflowView = () => {
  ...
  const handleSendEmail = async (supplierName: string, items: any[]) => {
    ...
    // 🚀 ดึงชื่อผู้ใช้งานที่ Login อยู่ปัจจุบันจาก LocalStorage
    ...
    try {
      const logRef = await addDoc(collection(db, 'email_logs'), {
        ...
        sent_by: currentUserName, // 🚀 เพิ่มชื่อคนกดส่ง Email ลงใน Log
      });
      ...
      items.forEach((item, index) => {
        ...
        body += `<span style="color:#10b981; font-weight:bold;">>>> จำนวนที่แนะนำให้ส่ง: ${
          orderQty > 0 ? orderQty.toLocaleString() : 'ตรวจสอบยอด'
        }</span>`;
        body += `</p>`;
      });
      body += `</div>`;
      ...
      body += `<span style="color:#94a3b8; font-size: 12px;">(ผู้ทำรายการ: ${currentUserName})</span></p>`; // 🚀 โชว์ชื่อคนส่งในท้าย Email ด้วย
      body += `</div>`;
    ...
    <PageTemplate title="Workflow & Replenishment">
      <div className="bg-white p-4 md:p-6 rounded-2xl shadow-sm border border-slate-200 flex-1 flex flex-col overflow-hidden">
        <div className="space-y-6 flex flex-col h-full">
          <div>
            <h3 className="text-xl font-bold text-slate-800 flex items-center gap-2">
              <GitMerge className="text-indigo-500" /> ระบบแจ้งอนุมัติและสั่งของ
              (PO/PR)
            </h3>
            <p className="text-sm text-slate-500 mt-1">
              กดปุ่มส่ง Email เพื่อแจ้ง Supplier อัตโนมัติ
              (ระบบจะลดแจ้งเตือนให้ทันทีที่ส่งสำเร็จ)
            </p>
          </div>
          ...
          {Object.keys(ordersBySupplier).length === 0 ? (
            <div className="bg-white border border-slate-200 rounded-2xl p-10 flex flex-col items-center justify-center text-center">
              ...
              <h4 className="text-xl font-bold text-slate-700">
                สต็อกสินค้าอยู่ในเกณฑ์ปกติ
              </h4>
              <p className="text-slate-500 mt-2">
                ยังไม่มีสินค้าที่ต่ำกว่าจุดสั่งซื้อ (Min Stock) ในขณะนี้
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 overflow-y-auto custom-scrollbar pb-10">
              {Object.entries(ordersBySupplier).map(([supplierName, items]) => {
                ...
                return (
                  <div
                    key={supplierName}
                    className="bg-white border border-indigo-100 rounded-2xl shadow-sm overflow-hidden flex flex-col"
                  >
                    <div className="bg-indigo-50/50 p-4 border-b border-indigo-100 flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-sm text-indigo-600 flex items-center gap-1 mt-1">
                          <Mail size={14} />{' '}
                          {hasEmail
                            ? matchedSup.supplier_email
                            : 'ยังไม่ได้ตั้งค่า Email'}
                        </p>
                      </div>

                      {hasUnemailed ? (
                        <span className="bg-red-100 text-red-700 text-xs font-bold px-3 py-1 rounded-full animate-pulse shrink-0 border border-red-200">
                          รอส่งแจ้งเตือน {unemailedItems.length} รายการ
                        </span>
                      ) : hasUnacknowledged ? (
                        <span className="bg-orange-100 text-orange-700 text-xs font-bold px-3 py-1 rounded-full animate-pulse shrink-0 border border-orange-200 flex items-center gap-1">
                          <Clock size={12} /> รอการตอบรับ{' '}
                          {unacknowledgedItems.length} รายการ
                        </span>
                      ) : (
                        <span className="bg-emerald-100 text-emerald-700 text-xs font-bold px-3 py-1 rounded-full shrink-0 flex items-center gap-1 border border-emerald-200">
                          <CheckCheck size={14} /> รับทราบครบแล้ว
                        </span>
                      )}
                    </div>
                    ...
                    <div className="p-4 flex-1 overflow-y-auto max-h-[250px] bg-slate-50/30">
                      <ul className="space-y-3">
                        {items.map((item, idx) => (
                          <li
                            key={idx}
                            className={`bg-white p-3 rounded-xl border shadow-sm flex justify-between items-center gap-2 ${
                    ...
                    <div className="p-4 border-t border-slate-100 bg-white">
                      {latestDate && (
                        <p className="text-xs text-slate-500 mb-3 font-bold flex items-center justify-center gap-1">
                          ส่ง Email ล่าสุดเมื่อ: {latestDate}
                        </p>
                      )}
                      ...
                      <button
                        ...
                        className={`w-full flex items-center justify-center gap-2 font-bold py-2.5 rounded-xl transition-all shadow-sm border 
                          ${
                            isSending
                              ? 'bg-indigo-400 cursor-not-allowed text-white'
                              : !hasEmail || isAllAcknowledged
                      ...
                      >
                        {isSending ? (
                          ...
                          <>
                            <CheckCheck size={18} /> ซัพพลายเออร์รับทราบครบแล้ว
                          </>
                        ) : hasUnemailed ? (
                          <>
                            <Send size={18} /> ส่ง Email แจ้ง Supplier
                          </>
                        ) : (
                          <>
                            <RefreshCw size={18} /> ส่ง Email แจ้งซ้ำอีกครั้ง
                          </>
                        )}
                      </button>
                      {!hasEmail && (
                        <p className="text-xs text-center text-red-500 mt-2">
                          กรุณาเพิ่ม Email ในหน้า Supplier Management ก่อน
                        </p>
                      )}
                    </div>
                  </div>
...
const SupplierManagementView = () => {
  ...
  const defaultForm = {
    ...
    uom: '',
    where_used: '',
    yearly_data: {} as Record<string, Record<string, number>>,
  };
  ...
  const getUniqueValues = (field: string) =>
    Array.from(new Set(suppliers.map((s) => s[field]).filter(Boolean)));
  ...
  const startEdit = (sup: any) => {
    setFormData({
      vendor_no: sup.vendor_no || '',
      vendor_name: sup.vendor_name || '',
      supplier_email: sup.supplier_email || '',
      part_no: sup.part_no || '',
      description: sup.description || '',
      uom: sup.uom || '',
      where_used: sup.where_used || '',
      yearly_data: sup.yearly_data || sup.monthly_data || {},
    });
    setEditingId(sup.id);
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm('ลบข้อมูลนี้ใช่หรือไม่?'))
  ...
  const handleVendorNoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const matchedSupplier = suppliers.find(
      (s) => s.vendor_no === inputVal && s.vendor_name
    );
    setFormData((prev) => ({
      ...prev,
      vendor_no: inputVal,
      vendor_name: matchedSupplier
        ? matchedSupplier.vendor_name
        : prev.vendor_name,
      supplier_email: matchedSupplier
  ...
  const filteredSuppliers = suppliers.filter((sup) => {
    ...
    return (
      sup.vendor_name?.toLowerCase().includes(term) ||
      sup.part_no?.toLowerCase().includes(term) ||
      sup.vendor_no?.toLowerCase().includes(term)
    );
  });
      ...
      <div className="bg-white p-4 md:p-6 rounded-2xl shadow-sm border border-slate-200 flex-1 flex flex-col overflow-hidden">
        <div className="space-y-6 flex flex-col h-full">
          <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 shrink-0">
            <div>
              <h3 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                <Users className="text-emerald-500" /> Supplier & Forecast
              </h3>
              <p className="text-sm text-slate-500 mt-1">
                จัดการฐานข้อมูลและประวัติการเบิกใช้ย้อนหลัง
              </p>
            </div>
            <div className="flex flex-col md:flex-row gap-3 w-full xl:w-auto items-stretch md:items-center">
              ...
              <div className="relative w-full md:w-64">
                ...
                <input
                  type="text"
                  placeholder="ค้นหา Vendor, P/N..."
                  className="w-full pl-10 pr-4 py-2 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm"
                  value={searchTerm}
              ...
              <button
                ...
                className="flex-1 md:flex-none justify-center bg-emerald-500 hover:bg-emerald-600 text-white px-4 py-2 rounded-xl text-sm font-bold flex items-center gap-2 transition-colors shadow-sm"
              >
          ...
          {isAdding && (
            <form
              onSubmit={handleSave}
              className="bg-emerald-50/50 p-6 rounded-2xl border border-emerald-200 grid grid-cols-1 md:grid-cols-4 xl:grid-cols-6 gap-4 animate-slide-down shrink-0 shadow-inner"
            >
              <div className="md:col-span-4 xl:col-span-6 border-b border-emerald-200 pb-2 mb-2">
                <h4 className="font-bold text-emerald-800 flex items-center gap-2">
                  ...
                  {editingId
                    ...
                    : 'เพิ่มข้อมูล Supplier ใหม่'}
                </h4>
              </div>

              <datalist id="vendorNos">
                {uniqueVendorNos.map((v) => (
              ...
              <datalist id="whereUseds">
                {uniqueWhereUseds.map((v) => (
                  <option key={v as string} value={v as string} />
                ))}
              </datalist>
              ...
              <div>
                ...
                <input
                  required
                  list="vendorNos"
                  className="w-full border border-slate-300 p-2 rounded-lg outline-none focus:ring-2 focus:ring-emerald-400"
              ...
              <div className="md:col-span-2">
                ...
                <input
                  list="vendorNames"
                  className="w-full border border-slate-300 p-2 rounded-lg outline-none focus:ring-2 focus:ring-emerald-400 bg-white"
              ...
              <div className="md:col-span-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, where_used: e.target.value })
                  }
                />
              </div>
              ...
              <div className="md:col-span-4 xl:col-span-6 mt-2 bg-white p-4 rounded-xl border border-emerald-100 shadow-sm">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-emerald-100 pb-3 mb-4 gap-2">
                  <label className="text-sm font-bold text-emerald-800">
                    บันทึก Forecast / Consumption ของปีนั้นๆ
                  </label>
                  <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 px-3 py-1.5 rounded-lg">
                    <span className="text-xs font-bold text-slate-500">
                      เลือกปีที่ต้องการบันทึก:
                    </span>
                    <select
                ...
                <div className="grid grid-cols-4 md:grid-cols-6 xl:grid-cols-12 gap-2 mb-4">
                  {MONTHS.map((month) => (
                    <div key={month}>
                      <label className="block text-[10px] font-bold text-slate-500 text-center uppercase mb-1">
                        {month}
                      </label>
                      <input
                        type="number"
                        step="any"
                        className="w-full border border-slate-300 p-1.5 rounded text-center text-sm outline-none focus:ring-2 focus:ring-emerald-400 bg-slate-50"
                        placeholder="0"
                        value={formData.yearly_data?.[formYear]?.[month] || ''}
                        onChange={(e) =>
                          handleYearlyChange(month, e.target.value)
                        }
                      />
                    </div>
                  ))}
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t border-emerald-50">
                  <div>
                    <label className="block text-[11px] font-bold text-red-700 mb-1">
                      Min Stock ปี {formYear} (Avg Wk × 2)
                    </label>
                    <input
                      type="text"
                      disabled
                      className="w-full border border-red-200 p-2 rounded-lg bg-red-50 text-red-600 font-bold cursor-not-allowed"
                      value={
                        formAutoMin > 0
                          ? formAutoMin.toLocaleString(undefined, {
                              maximumFractionDigits: 1,
                            })
                          : '0'
                      }
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-emerald-700 mb-1">
                      Max Stock ปี {formYear} (Avg Wk × 4)
                    </label>
                    <input
                      type="text"
                      disabled
                      className="w-full border border-emerald-200 p-2 rounded-lg bg-emerald-50 text-emerald-600 font-bold cursor-not-allowed"
                      value={
                        formAutoMax > 0
                          ? formAutoMax.toLocaleString(undefined, {
                              maximumFractionDigits: 1,
                            })
                          : '0'
                      }
              ...
              <div className="md:col-span-4 xl:col-span-6 flex justify-end gap-2 mt-2">
                ...
                <button
                  type="submit"
                  className="bg-emerald-600 text-white px-8 py-2.5 rounded-xl font-bold hover:bg-emerald-700 shadow-md transition-colors"
                >
            ...
            <div className="overflow-x-auto overflow-y-auto flex-1 custom-scrollbar">
              <table className="w-full text-left whitespace-nowrap text-sm min-w-max">
                <thead className="bg-slate-100 text-slate-600 text-[11px] uppercase font-bold border-b border-slate-200 sticky top-0 z-10 shadow-sm">
                  <tr>
                    <th className="p-3 sticky left-0 bg-slate-100 z-20 shadow-[2px_0_5px_rgba(0,0,0,0.02)]">
                      Vendor No
                    </th>
                    ...
                    {MONTHS.map((m) => (
                      <th
                        key={m}
                        className="p-3 text-center text-emerald-700 bg-emerald-50/50 border-l border-white"
                      >
                        {m}
                      </th>
                    ))}
                    <th className="p-3 text-center bg-blue-50 text-blue-800 border-l border-white">
                      Avg Month
                    </th>
                    <th className="p-3 text-center bg-blue-50 text-blue-800">
                      Avg Week
                    </th>
                    <th className="p-3 border-l border-slate-200">UOM</th>
                    <th className="p-3">Where Used</th>
                    <th className="p-3 text-center text-red-700 bg-red-50/30">
                      Min (Wkx2)
                    </th>
                    <th className="p-3 text-center text-emerald-700 bg-emerald-50/30">
                      Max (Wkx4)
                    </th>
                    <th className="p-3 text-center sticky right-0 bg-slate-100 shadow-[-2px_0_5px_rgba(0,0,0,0.02)] z-20">
                      Manage
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {filteredSuppliers.length === 0 ? (
                    ...
                    filteredSuppliers.map((sup) => {
                      const yearData =
                        sup.yearly_data?.[selectedYear] ||
                        sup.monthly_data?.[selectedYear] ||
                        {};
                      ...
                      return (
                        ...
                        >
                          <td className="p-3 font-mono font-bold text-slate-700 sticky left-0 bg-white shadow-[2px_0_5px_rgba(0,0,0,0.02)] z-10">
                            {sup.vendor_no}
                          </td>
                          ...
                          {MONTHS.map((m) => (
                            <td
                              key={m}
                              className="p-3 text-center font-mono text-slate-600 border-l border-slate-50"
                            >
                              {yearData[m] > 0
                                ? yearData[m].toLocaleString(undefined, {
                                    maximumFractionDigits: 2,
                          ...
                          <td className="p-3 text-center font-mono font-bold text-blue-700 bg-blue-50/20 border-l border-slate-100">
                            {avgMonth > 0
                              ? avgMonth.toLocaleString(undefined, {
                                  maximumFractionDigits: 1,
                                })
                          ...
                          <td className="p-3 text-center font-mono font-bold text-blue-700 bg-blue-50/20">
                            {avgWeek > 0
                              ? avgWeek.toLocaleString(undefined, {
                                  maximumFractionDigits: 1,
                                })
                          ...
                          <td className="p-3 text-slate-500 border-l border-slate-100">
                            {sup.uom}
                          </td>
                          ...
                          <td className="p-3 text-center font-mono font-bold text-red-600 bg-red-50/10 border-l border-slate-50">
                            {autoMin > 0
                              ? autoMin.toLocaleString(undefined, {
                                  maximumFractionDigits: 1,
                                })
                          ...
                          <td className="p-3 text-center font-mono font-bold text-emerald-600 bg-emerald-50/10 border-l border-slate-50">
                            {autoMax > 0
                              ? autoMax.toLocaleString(undefined, {
                                  maximumFractionDigits: 1,
                                })
...
const StockManagementView = () => {
  ...
  // 🚀 State สำหรับเก็บเวลาอัปโหลดล่าสุด
  ...
  // 🚀 ดึงเวลาอัปโหลด Excel ล่าสุดจาก Firebase
  useEffect(() => {
    const unsubMeta = onSnapshot(doc(db, 'system', 'meta'), (docSnap) => {
      if (docSnap.exists()) {
        const data = docSnap.data();
        if (data.last_stock_upload) {
          setLastUploadTime(data.last_stock_upload);
        }
      }
    });
    return () => unsubMeta();
  ...
  const handleAutoSync = async () => {
    ...
    for (const product of products) {
      ...
      if (matchedSup) {
        ...
        if (
          product.supplier_name !== vendorName ||
          product.min_stock !== latestMin
        ) {
    ...
    if (syncedCount > 0)
      alert(
        `✅ ผูกข้อมูลสำเร็จ! ระบบอัปเดตให้ ${syncedCount} รายการเรียบร้อยแล้ว`
      );
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (const row of jsonData) {
        ...
        if (part_no) {
          parsedData.push({
            ...
            on_hand_qty:
              Number(String(getVal('On Hand Qty') || '0').replace(/,/g, '')) ||
              0,
            supply:
              Number(String(getVal('Supply') || '0').replace(/,/g, '')) || 0,
            demand:
              Number(String(getVal('Demand') || '0').replace(/,/g, '')) || 0,
            available_qty:
              Number(
                String(getVal('Available Qty') || '0').replace(/,/g, '')
              ) || 0,
          });
        }
      }
      ...
      for (const item of parsedData) {
        ...
        const supName = matchedSup
          ? matchedSup.vendor_name || 'ไม่ระบุชื่อ'
          : 'รอการผูกข้อมูล';
      ...
      // ... โค้ดช่วง loop บันทึก items ด้านบนยังเหมือนเดิม ...

      // 🚀 1. บันทึกเวลาที่อัปโหลดล่าสุด
      await setDoc(
        doc(db, 'system', 'meta'),
        {
          last_stock_upload: serverTimestamp(),
        },
        { merge: true }
      );

      // 🚀 ดึงชื่อผู้ใช้งานที่กำลัง Login อยู่
      let currentUserName = 'System';
      try {
        const userStr = localStorage.getItem('vmi_user');
        if (userStr) currentUserName = JSON.parse(userStr).name;
      ...
      // 🚀 2. เก็บประวัติ Log การ Upload ไฟล์ โดยใช้ชื่อคนล็อกอินจริง
  ...
  }; // สิ้นสุดฟังก์ชัน handleFileUpload
  ...
  const handleSaveProduct = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        ...
        await addDoc(collection(db, 'products'), {
          ...payload,
          supplier_name: formData.supplier_name || 'รอการผูกข้อมูล',
          createdAt: serverTimestamp(),
        });
      }
      setFormData(defaultForm);
  ...
  const filteredProducts = products.filter((p) => {
    ...
    return (
      matchSearch && (showOnlyLowStock ? p.available_qty <= p.min_stock : true)
    );
  });
      ...
      <div className="bg-white p-4 md:p-6 rounded-2xl shadow-sm border border-slate-200 flex-1 flex flex-col overflow-hidden">
        <div className="space-y-6 flex flex-col h-full relative">
          <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 shrink-0">
            <div>
              <h3 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                <PackageSearch className="text-orange-500" /> ERP Inventory Sync
              </h3>
              <div className="flex items-center gap-3 mt-1">
                <p className="text-sm text-slate-500">
                  จัดการสต็อกและรายการสินค้า (VMI)
                </p>
                {/* 🚀 ป้ายแสดงเวลาอัปโหลดล่าสุด */}
                {lastUploadTime && (
                  <span className="text-[10px] bg-blue-50 text-blue-600 px-2 py-0.5 rounded-full font-bold flex items-center gap-1 border border-blue-200">
                    <Clock size={12} /> อัปเดตล่าสุด:{' '}
                    {formatTrackingDate(lastUploadTime)}
                  </span>
                )}
              </div>
            </div>
            <div className="flex flex-col md:flex-row gap-3 w-full xl:w-auto items-stretch md:items-center">
              <div className="relative w-full md:w-56">
                ...
                <input
                  type="text"
                  placeholder="ค้นหา Part No, ซัพพลายเออร์"
              ...
              <button
                ...
                } text-white px-4 py-2 rounded-xl text-sm font-bold flex items-center gap-2 transition-colors whitespace-nowrap`}
              >
                {isSyncing ? (
                  <Clock className="animate-spin" size={18} />
                ) : (
                ...
                <span>ผูก Supplier</span>
              </button>
              <label
                className={`justify-center ${
                  isImporting ? 'bg-slate-400' : 'bg-blue-600 hover:bg-blue-700'
                } text-white px-4 py-2 rounded-xl text-sm font-bold flex items-center gap-2 cursor-pointer whitespace-nowrap`}
              >
                <UploadCloud size={18} />
                <span>นำเข้า ERP</span>
                <input
                  type="file"
                  accept=".csv, .xlsx, .xls"
                  className="hidden"
              ...
              <button
                ...
                className="justify-center bg-orange-500 hover:bg-orange-600 text-white px-4 py-2 rounded-xl text-sm font-bold flex items-center gap-2 whitespace-nowrap"
              >
          ...
          {allLowStock.length > 0 && (
            <div
              ...
              className={`cursor-pointer p-4 rounded-2xl border flex items-center justify-between shadow-sm shrink-0 ${
                showOnlyLowStock
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm">
                    พบสินค้า{' '}
                    <span className="font-bold">{allLowStock.length}</span>{' '}
                    รายการที่ต่ำกว่า Min Stock
                    <span
                      className={`ml-2 px-2 py-0.5 rounded text-xs font-bold ${
                        showOnlyLowStock ? 'bg-white/30' : 'bg-red-200'
                      }`}
                    >
              ...
              <div
                className={`font-bold text-sm px-4 py-2 rounded-xl ${
                  showOnlyLowStock ? 'bg-black/20' : 'bg-red-600 text-white'
                }`}
              >
          ...
          {isAdding && (
            <form
              ...
              className="bg-orange-50/50 p-6 rounded-2xl border border-orange-200 grid grid-cols-1 md:grid-cols-4 gap-4 shrink-0 animate-slide-down"
            >
              ...
              <div>
                ...
                <input
                  required
                  disabled={!!editingId}
                  className="w-full border p-2.5 rounded-xl outline-none disabled:bg-slate-100"
                  placeholder="เช่น 100085"
                  value={formData.part_no}
                  onChange={(e) =>
                    setFormData({ ...formData, part_no: e.target.value })
                  }
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, config_id: e.target.value })
                  }
                />
              </div>
              <div>
              ...
              <div>
                ...
                <input
                  className="w-full border p-2.5 rounded-xl outline-none focus:ring-2 focus:ring-orange-400"
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, on_hand_qty: e.target.value })
                  }
                />
              </div>
              <div>
              ...
              <div className="bg-blue-50 p-2 rounded-xl">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, available_qty: e.target.value })
                  }
                />
              </div>
              ...
              <div className="md:col-span-2 flex justify-end gap-2 mt-4 items-end">
                <button
                  ...
                  className="px-6 py-2.5 text-slate-600 hover:bg-slate-200 bg-slate-100 rounded-xl font-bold transition-colors"
                >
                  ยกเลิก
                </button>
                <button
                  type="submit"
                  className="bg-orange-600 text-white px-8 py-2.5 rounded-xl font-bold hover:bg-orange-700 shadow-md transition-colors"
                >
                  {editingId ? 'อัปเดตข้อมูล' : 'บันทึกข้อมูล'}
                </button>
              </div>
              ...
              <table className="w-full text-left whitespace-nowrap text-sm min-w-max">
                <thead className="bg-slate-100 text-slate-600 text-[11px] uppercase font-bold sticky top-0 z-10 shadow-sm">
                  <tr>
                    ...
                    <th className="p-3 text-right text-blue-700 bg-blue-50/50">
                      Available Qty
                    </th>
                    <th className="p-3 border-l border-slate-200">Supplier</th>
                    <th className="p-3 text-center">Min Stock</th>
                    <th className="p-3 text-center">Status</th>
                    <th className="p-3 text-center border-l border-white bg-orange-50 text-orange-800">
                      Reorder date
                    </th>
                    <th className="p-3 text-center sticky right-0 bg-slate-100 shadow-[-5px_0_10px_rgba(0,0,0,0.02)] z-20">
                      Manage
                    </th>
                  </tr>
                </thead>
                    ...
                    return (
                      ...
                      >
                        <td className="p-3 font-mono font-bold text-slate-700 sticky left-0 bg-white shadow-[2px_0_5px_rgba(0,0,0,0.02)] z-10 flex items-center gap-2">
                          ...
                          {p.is_acknowledged ? (
                            <span
                              className="text-[9px] bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded border border-emerald-200"
                              title="รับทราบแล้ว"
                            >
                              Acked
                            </span>
                            ...
                            <span
                              className="text-[9px] bg-orange-100 text-orange-700 px-1.5 py-0.5 rounded border border-orange-200"
                              title="รอการตอบรับ"
                            >
                              Sent
                            </span>
                        ...
                        <td className="p-3 text-slate-500">{p.site}</td>
                        <td className="p-3 text-slate-500">{p.config_id}</td>
                        <td className="p-3 text-slate-500">{p.project_id}</td>
                        <td className="p-3 text-right font-mono text-slate-600">
                          {p.on_hand_qty?.toLocaleString(undefined, {
                            maximumFractionDigits: 2,
                          })}
                        </td>
                        <td className="p-3 text-right font-mono text-slate-600">
                          {p.supply?.toLocaleString(undefined, {
                            maximumFractionDigits: 2,
                          })}
                        </td>
                        <td className="p-3 text-right font-mono text-slate-600">
                          {p.demand?.toLocaleString(undefined, {
                            maximumFractionDigits: 2,
                          })}
                        </td>
                        <td className="p-3 text-right font-mono font-bold text-blue-600 bg-blue-50/20">
                          {p.available_qty?.toLocaleString(undefined, {
                            maximumFractionDigits: 2,
                          })}
                        </td>
                        <td className="p-3 border-l border-slate-50">
                          {p.supplier_name === 'รอการผูกข้อมูล' ? (
                            <span className="text-orange-500 italic bg-orange-50 px-2 rounded text-xs">
                              รอการผูก
                            </span>
                          ) : (
                            <span className="text-emerald-700 font-bold bg-emerald-50 px-2 rounded text-xs">
                              {p.supplier_name}
                            </span>
                          )}
                        </td>
                        <td className="p-3 text-center font-mono font-bold text-red-500 border-l border-slate-50">
                          {p.min_stock?.toLocaleString(undefined, {
                            maximumFractionDigits: 1,
                          })}
                        </td>
                        <td className="p-3 text-center">
                        ...
                        <td className="p-3 text-center border-l border-slate-50">
                          <span
                            className={`px-2 py-1 rounded text-[10px] font-bold inline-block w-full ${
                              forecast.urgent
          ...
          {/* Modal: Pop-up ดูรายละเอียด Consumption */}
          {viewingConsumptionPart && (
            <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
              <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl overflow-hidden flex flex-col max-h-[90vh] animate-slide-down">
                <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
...
const ReportsView = () => {
  ...
  const [logs, setLogs] = useState<any[]>([]);
  const [uploadLogs, setUploadLogs] = useState<any[]>([]); // 🚀 State สำหรับเก็บประวัติอัปโหลด
  ...
  const reportList = [
    ...
    {
      id: 'upload_logs',
      title: 'ประวัติการอัปโหลด (Upload Logs)',
      icon: UploadCloud,
    },
    {
      id: 'stock_value',
      title: 'มูลค่าสต็อกคงเหลือ (เร็วๆ นี้)',
      icon: DollarSign,
    },
    {
      id: 'supplier_perf',
      title: 'ประสิทธิภาพ Supplier (เร็วๆ นี้)',
      icon: TrendingUp,
    },
    {
      id: 'forecast_demand',
      title: 'แนวโน้มการเบิกใช้ (เร็วๆ นี้)',
      icon: BarChart3,
    },
  ];

  useEffect(() => {
    // 1. ดึงข้อมูลประวัติส่งอีเมล
    const qEmail = query(
    ...
    // 🚀 2. ดึงข้อมูลประวัติการอัปโหลดไฟล์
    const qUpload = query(
  ...
  return (
    <PageTemplate title="Reports & Analytics">
      <div className="flex flex-col lg:flex-row gap-6 h-full p-4 md:p-6 max-h-[calc(100vh-64px)] overflow-hidden">
        ...
        <div className="w-full lg:w-72 bg-white border border-slate-200 rounded-2xl p-4 shrink-0 shadow-sm flex flex-col">
          ...
          <div className="space-y-2 overflow-y-auto">
            {reportList.map((rep) => {
              ...
              const isFuture =
                rep.id !== 'email_logs' && rep.id !== 'upload_logs'; // ล็อกเมนูที่ยังไม่เปิดใช้
              return (
                <button
                  key={rep.id}
        ...
        {/* พื้นที่แสดงรายงาน (Main Content) */}
        <div className="flex-1 bg-white border border-slate-200 rounded-2xl shadow-sm flex flex-col overflow-hidden relative">
          {/* ===================================== */}
          {/* 1. รายงาน: ประวัติการส่งอีเมล */}
          {/* ===================================== */}
          {activeReport === 'email_logs' && (
            <>
              <div className="p-4 md:p-6 border-b border-slate-100 bg-slate-50/50">
                <h3 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                  <History className="text-indigo-600" /> ประวัติการส่งแจ้งเตือน
                  (Email Logs)
                </h3>
                <p className="text-sm text-slate-500 mt-1">
                  ประวัติการส่งใบสั่งของและสถานะการรับทราบจาก Supplier
                </p>
              </div>
              ...
              <div className="flex-1 overflow-x-auto overflow-y-auto custom-scrollbar p-0">
                <table className="w-full text-left whitespace-nowrap text-sm min-w-max">
                  <thead className="bg-slate-100 text-slate-600 text-[11px] uppercase font-bold sticky top-0 z-10 shadow-sm">
                    <tr>
                      <th className="p-4 pl-6">วันที่ส่ง (Date/Time)</th>
                      <th className="p-4">ผู้ส่ง (Sent By)</th>{' '}
                      {/* 🚀 เพิ่มคอลัมน์ผู้ส่ง */}
                      <th className="p-4">ชื่อซัพพลายเออร์ (Supplier)</th>
                      <th className="p-4 text-center">จำนวนที่แจ้ง</th>
                      <th className="p-4 text-center">สถานะตอบรับ</th>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {logs.length === 0 ? (
                      <tr>
                        ...
                        >
                          ยังไม่มีประวัติการส่ง Email ในระบบ
                        </td>
                      </tr>
                    ) : (
                      logs.map((log) => {
                          ...
                          >
                            ...
                            <td className="p-4 text-slate-600">
                              <div className="flex items-center gap-2">
                                <div className="w-6 h-6 rounded-full bg-indigo-100 flex items-center justify-center text-[10px] font-bold text-indigo-600">
                                  {log.sent_by
                                    ? log.sent_by.charAt(0).toUpperCase()
                                    : 'S'}
                                </div>
                                {log.sent_by || 'System'}
                              </div>
                              ...
                              {log.acknowledged ? (
                                <div className="flex flex-col items-center">
                                  <span className="bg-emerald-100 text-emerald-700 px-2 py-1 rounded text-xs font-bold flex items-center gap-1">
                                    <CheckCircle size={12} /> รับทราบแล้ว
                                  </span>
                                  <span className="text-[10px] text-slate-400 mt-1 font-mono">
                                    {log.acknowledged_at?.toDate
                                      ? log.acknowledged_at
                                          .toDate()
                                          .toLocaleString('th-TH')
                                      : ''}
                            ...
                            <td className="p-4 text-center pr-6">
                              <button
                                ...
                                className="inline-flex items-center gap-1 bg-slate-100 hover:bg-slate-200 text-slate-700 px-3 py-1.5 rounded-lg text-xs font-bold transition-colors"
                              >
          ...
          {/* ===================================== */}
          {/* 🚀 2. รายงาน: ประวัติการอัปโหลดไฟล์ Excel */}
          {/* ===================================== */}
          {activeReport === 'upload_logs' && (
            <>
              <div className="p-4 md:p-6 border-b border-slate-100 bg-slate-50/50">
                <h3 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                  <UploadCloud className="text-blue-600" /> ประวัติการนำเข้า ERP
                  (Upload Logs)
                </h3>
                <p className="text-sm text-slate-500 mt-1">
                  ประวัติการอัปโหลดไฟล์ Excel เพื่ออัปเดตยอดสต็อกและจำนวนสินค้า
                </p>
              </div>
              ...
              <div className="flex-1 overflow-x-auto overflow-y-auto custom-scrollbar p-0">
                <table className="w-full text-left whitespace-nowrap text-sm min-w-max">
                  <thead className="bg-slate-100 text-slate-600 text-[11px] uppercase font-bold sticky top-0 z-10 shadow-sm">
                    <tr>
                      <th className="p-4 pl-6">วันที่และเวลา (Date/Time)</th>
                      <th className="p-4 text-center">รวมทั้งหมด (Total)</th>
                      ...
                      <th className="p-4 pr-6">ผู้ทำรายการ (By)</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {uploadLogs.length === 0 ? (
                      ...
                      uploadLogs.map((log) => {
                        const uploadDate = log.uploaded_at?.toDate
                          ? log.uploaded_at.toDate().toLocaleString('th-TH')
                          : 'กำลังบันทึก...';
                        return (
                          <tr
                            key={log.id}
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl overflow-hidden flex flex-col max-h-[80vh] animate-slide-down">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="font-bold text-lg text-slate-800">
                      รายละเอียดใบแจ้งเตือน
                    </h3>
                    <p className="text-sm text-slate-500">
                      ส่งถึง: {viewingLogDetails.supplier_name} (
                      {viewingLogDetails.supplier_email})
                    </p>
                  </div>
                </div>
                <button
              ...
              <div className="p-6 overflow-y-auto custom-scrollbar flex-1 bg-slate-50/50">
                <p className="text-sm font-bold text-slate-700 mb-3">
                  รายการสินค้าที่แจ้งให้มาส่ง (ทั้งหมด{' '}
                  {viewingLogDetails.items?.length} รายการ):
                </p>
...
const SettingsView = () => {
  ...
  useEffect(() => {
    // ดึงข้อมูลรายชื่อผู้ใช้งานจาก Firebase
  ...
  const handleSaveUser = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      // 💡 Mock ข้อมูลลงตาราง (ในอนาคตจะเชื่อมกับ Firebase Auth)
      await addDoc(collection(db, 'users'), {
        name: formData.name,
        email: formData.email,
        role: formData.role,
        password: formData.password, // 🚀 แก้บรรทัดนี้ ให้บันทึกรหัสผ่านจริง
        status: 'Active',
        createdAt: serverTimestamp(),
      });
      setFormData({ name: '', email: '', role: 'User', password: '' });
      setIsAddingUser(false);
      alert('เพิ่มผู้ใช้งานสำเร็จ! (รอการเชื่อมต่อระบบ Login ในเฟสถัดไป)');
    } catch (e) {
      alert('เกิดข้อผิดพลาดในการบันทึกข้อมูล');
    }
  };
        ...
        <div className="w-full lg:w-72 bg-white border border-slate-200 rounded-2xl p-4 shrink-0 shadow-sm flex flex-col">
          ...
          <div className="space-y-2 overflow-y-auto">
            ...
            >
              <Users
                size={18}
                className={activeTab === 'users' ? 'text-blue-400' : ''}
              />
            ...
            >
              <Settings
                size={18}
                className={activeTab === 'general' ? 'text-emerald-400' : ''}
              />
            ...
            <>
              <div className="p-4 md:p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                <div>
                  <h3 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                    ...
                    จัดการสิทธิ์และผู้ใช้งาน
                  </h3>
                  <p className="text-sm text-slate-500 mt-1">
                    เพิ่มหรือลบพนักงานที่สามารถเข้าใช้งานระบบ VMI ได้
                  </p>
                </div>
                <button
              ...
              {isAddingUser && (
                <form
                  ...
                  className="p-6 bg-blue-50/50 border-b border-blue-100 grid grid-cols-1 md:grid-cols-2 gap-4 animate-slide-down shrink-0"
                >
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1">
                      ชื่อ - นามสกุล *
                    ...
                    <input
                      required
                      type="text"
                      className="w-full border border-slate-300 p-2.5 rounded-lg outline-none focus:ring-2 focus:ring-blue-400"
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        type="password"
                        minLength={6}
                        className="w-full border border-slate-300 pl-9 pr-4 py-2.5 rounded-lg outline-none focus:ring-2 focus:ring-blue-400"
                  ...
                  <div>
                    ...
                    <select
                      className="w-full border border-slate-300 p-2.5 rounded-lg outline-none focus:ring-2 focus:ring-blue-400 font-bold text-slate-700 bg-white"
                      value={formData.role}
                      onChange={(e) =>
                  ...
                  <div className="md:col-span-2 flex justify-end gap-2 mt-2">
                    ...
                    <button
                      type="submit"
                      className="bg-blue-600 text-white px-8 py-2.5 rounded-xl font-bold hover:bg-blue-700 shadow-md transition-colors"
                    >
                      บันทึกข้อมูลผู้ใช้
                    </button>
                  </div>
              ...
              <div className="flex-1 overflow-x-auto overflow-y-auto custom-scrollbar p-0">
                <table className="w-full text-left whitespace-nowrap text-sm min-w-max">
                  <thead className="bg-slate-100 text-slate-600 text-[11px] uppercase font-bold sticky top-0 z-10 shadow-sm">
                    <tr>
                      <th className="p-4 pl-6">ชื่อ (Name)</th>
                    ...
                    {usersList.length === 0 ? (
                      ...
                      usersList.map((u) => (
                        ...
                        >
                          ...
                          <td className="p-4 text-center">
                            <span className="bg-emerald-100 text-emerald-700 px-2 py-1 rounded text-xs font-bold flex items-center justify-center gap-1 w-20 mx-auto">
          ...
          {activeTab === 'general' && (
            <div className="p-10 flex flex-col items-center justify-center h-full text-center">
              ...
              <p className="text-slate-500 max-w-md">
                พื้นที่นี้เตรียมไว้สำหรับการตั้งค่าอื่นๆ ในอนาคต เช่น
                ข้อมูลบริษัท, รูปแบบการแจ้งเตือน หรือการเชื่อมต่อ Line OA
              </p>
              <div className="mt-8 px-6 py-3 bg-slate-100 text-slate-500 rounded-xl font-bold text-sm border border-slate-200 border-dashed">
                🚧 Coming Soon
              </div>
            </div>
          )}
  ...
  return (
    <Router>
      <Routes>
        ...
        {/* 🚀 Protected Routes (ต้องล็อกอินทั้งหมด) */}
        <Route
          path="/"
          element={
            <ProtectedRoute>
